...
k1(1,i) = Vqsr(1,i)+(Rs*Iqsr(1,i))-(wr(1,i)*Lamdsr(1,i));
m1(1,i) = Vdsr(1,i)+(Rs*Idsr(1,i))+(wr(1,i)*Lamqsr(1,i));
n1(1,i) = Vos(1,i)+(Rs*Ios(1,i));

%% Calculations for k2, m2, n2:

t2(1,i) = t(1,i) + (h/2);

Lamqsr2(1,i) = Lamqsr(1,i) + (k1(1,i)*(h/2));
Lamdsr2(1,i) = Lamdsr(1,i) + (m1(1,i)*(h/2));
Lamos2(1,i) = Lamos(1,i) + (n1(1,i)*(h/2));

 k2(1,i) = Vqsr(1,i)+(Rs*Iqsr(1,i))-(wr(1,i)*Lamdsr2(1,i));
 m2(1,i) = Vdsr(1,i)+(Rs*Idsr(1,i))+(wr(1,i)*Lamqsr2(1,i));
 n2(1,i) = Vos(1,i)+(Rs*Ios(1,i));
...
Lamqsr3(1,i) = Lamqsr2(1,i) + (k2(1,i)*(h/2));
Lamdsr3(1,i) = Lamdsr2(1,i) + (m2(1,i)*(h/2));
Lamos3(1,i) = Lamos2(1,i) + (n2(1,i)*(h/2));

 k3(1,i) = Vqsr(1,i)+(Rs*Iqsr(1,i))-(wr(1,i)*Lamdsr3(1,i));
 m3(1,i) = Vdsr(1,i)+(Rs*Idsr(1,i))+(wr(1,i)*Lamqsr3(1,i));
 n3(1,i) = Vos(1,i)-(Rs*Ios(1,i));
 
%% Calculations for k4, m4, n4:
t4(1,i) = t(1,i) +  h;

Lamqsr4(1,i) = Lamqsr3(1,i) + (k3(1,i)*(h/2));
Lamdsr4(1,i) = Lamdsr3(1,i) + (m3(1,i)*(h/2));
Lamos4(1,i) = Lamos3(1,i) + (n3(1,i)*(h/2));
 
  k4(1,i) = Vqsr(1,i)+(Rs*Iqsr(1,i))-(wr(1,i)*Lamdsr4(1,i));
  m4(1,i) = Vdsr(1,i)+(Rs*Idsr(1,i))+(wr(1,i)*Lamqsr4(1,i));
  n4(1,i) = Vos(1,i)-(Rs*Ios(1,i));
  
%% Calculations for the next value of Lamqsr, Lamdsr, Lamos:

Lamqsr(1,i+1) = Lamqsr(1,i) + (h/6) * (k1(1,i) + 2*k2(1,i) + 2*k3(1,i) + k4(1,i));

Lamdsr(1,i+1) = Lamdsr(1,i) + (h/6) * (m1(1,i) + 2*m2(1,i) + 2*m3(1,i) + m4(1,i));
...
z1(1,i) = V_kq1r(1,i) - (R_kq1 * I_kq1r(1,i));
x1(1,i) = V_kq2r(1,i) - (R_kq2 * I_kq2r(1,i));
c1(1,i) = V_fdr(1,i) - (R_fd * I_fdr(1,i));
s1(1,i) = V_kdr(1,i) - (R_kd * I_kdr(1,i));

%% Calculations for z2, x2, c2, s2:

t2(1,i) = t(1,i) + (h/2);

Lam_kq1r2(1,i) = Lam_kq1r(1,i) + (z1(1,i) * (h/2));
Lam_kq2r2(1,i) = Lam_kq2r(1,i) + (x1(1,i) * (h/2));
Lam_fdr2(1,i) = Lam_fdr(1,i) + (c1(1,i) * (h/2));
Lam_kdr2(1,i) = Lam_kdr(1,i) + (s1(1,i) * (h/2));

 z2(1,i) = V_kq1r(1,i) - (R_kq1 * I_kq1r(1,i));
 x2(1,i) = V_kq2r(1,i) - (R_kq2 * I_kq2r(1,i));
 c2(1,i) = V_fdr(1,i) - (R_fd * I_fdr(1,i));
 s2(1,i) = V_kdr(1,i) - (R_kd * I_kdr(1,i));
...
Lam_kq1r3(1,i) = Lam_kq1r2(1,i) + (z2(1,i) * (h/2));
Lam_kq2r3(1,i) = Lam_kq2r2(1,i) + (x2(1,i) * (h/2));
Lam_fdr3(1,i) = Lam_fdr2(1,i) + (c2(1,i) * (h/2));
Lam_kdr3(1,i) = Lam_kdr2(1,i) + (s2(1,i) * (h/2));


 z3(1,i) = V_kq1r(1,i) - (R_kq1 * I_kq1r(1,i)); 
 x3(1,i) = V_kq2r(1,i) - (R_kq2 * I_kq2r(1,i));
 c3(1,i) = V_fdr(1,i) - (R_fd * I_fdr(1,i));
 s3(1,i) = V_kdr(1,i) - (R_kd * I_kdr(1,i));
 
%% Calculations for z4, x4, c4, s4:
t4(1,i) = t(1,i) +  h;

Lam_kq1r4(1,i) = Lam_kq1r3(1,i) + (z3(1,i) * (h/2));
Lam_kq2r4(1,i) = Lam_kq2r3(1,i) + (x3(1,i) * (h/2));
Lam_fdr4(1,i) = Lam_fdr3(1,i) + (c3(1,i) * (h/2));
Lam_kdr4(1,i) = Lam_kdr3(1,i) + (s3(1,i) * (h/2));

 
  z4(1,i) = V_kq1r(1,i) - (R_kq1 * I_kq1r(1,i));
  x4(1,i) = V_kq2r(1,i) - (R_kq2 * I_kq2r(1,i));
  c4(1,i) = V_fdr(1,i) - (R_fd * I_fdr(1,i));
  s4(1,i) = V_kdr(1,i) - (R_kd * I_kdr(1,i));
  
%% Calculations for the next value of Lamqr, Lamdr, Lamor:

Lam_kq1r(1,i+1) = Lam_kq1r(1,i) + (h/6) * (z1(1,i) + 2*z2(1,i) + 2*z3(1,i) + z4(1,i));

Lam_kq2r(1,i+1) = Lam_kq2r(1,i) + (h/6) * (x1(1,i) + 2*x2(1,i) + 2*x3(1,i) + x4(1,i));

Lam_fdr(1,i+1) = Lam_fdr(1,i) + (h/6) * (c1(1,i) + 2*c2(1,i) + 2*c3(1,i) + c4(1,i));

Lam_kdr(1,i+1) = Lam_kdr(1,i) + (h/6) * (s1(1,i) + 2*s2(1,i) + 2*s3(1,i) + s4(1,i));
%--------------------------------------------------------------------%  
    ...
    A = [-(Lls+Lmq) 0 0 Lmq Lmq 0 0; 0 -(Lls+Lmd) 0 0 0 Lmd Lmd; 0 0 -Lls 0 0 0 0; -Lmq 0 0 (Ll_kq1+Lmq) Lmq 0 0; -Lmq 0 0 Lmq (Ll_kq2+Lmq) 0 0; 0 -Lmd 0 0 0 (Ll_fd+Lmd) Lmd; 0 -Lmd 0 0 0 Lmd (Ll_kd+Lmd)];
    B = [Lamqsr(1,i+1); Lamdsr(1,i+1); Lamos(1,i+1); Lam_kq1r(1,i+1); Lam_kq2r(1,i+1); Lam_fdr(1,i+1); Lam_kdr(1,i+1)];
    X = linsolve(A, B);
